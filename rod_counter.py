...
def write_inventory_summary(path: str, summary: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        f"- 模型识别杆件段数 model_member_count: {summary['model_rod_count']} 段",
        f"- 逐段折算等效标准杆数量（未排料，不用于最终材料分）: {summary['equivalent_standard_count']} 根",
        f"- 基准数量: {summary['base_count']}",
        f"- 逐段折算是否超过 60 根: {'是' if summary['exceeds_base'] else '否'}",
        f"- 逐段折算材料分（旧口径，仅供检查）: {summary['material_score']}",
        "- 最终材料成本分请查看 material_stock_summary.md 或 analysis_report.md 中的 stock_wood_count。",
